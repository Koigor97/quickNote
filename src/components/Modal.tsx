...
/**
 * Context for managing the modal's open/close state.
 * Provides the `open` state and `setOpen` function to nested components.
 */
const ModalContext = createContext<ModalContextProps | undefined>(undefined);

/**
 * Custom hook to access the Modal context.
 * Throws an error if used outside of the `ModalContext.Provider`.
 *
 *
 *
 * @example
...
/**
 * FramerModal creates an animated modal that can be opened and closed.
 * - Closes on outside click or `Escape` key press.
 * - Prevents background scrolling when open.
 *
 * @param {ReactNode} children - The content inside the modal.
 * @param {boolean} [open] - Controlled state for opening the modal.
 * @param {(open: boolean) => void} [setOpen] - Function to toggle modal state.
 *
 *
 *
 * @example
...
/**
 * ModalContent wraps the content displayed inside the modal.
 *
 * @param {ReactNode} children - The content to render inside the modal.
 *
 *
 *
 * @example
